...
tool_fix_IEA_data_for_Industry_subsectors <- function(data, ieamatch) {
  ...
  .get_production <- function(type, pf, flow) {
    calcOutput(
      ...
      `[`(,,pf, pmatch  = 'right') %>%
      magclass_to_tibble() %>%
      filter(.data$year %in% unique(data_industry$year)) %>%
      group_by(!!!syms(c('iso3c', 'year', 'pf'))) %>%
      verify(expr = 0 != var(.data$value),
             description = paste('No scenarios differences of', pf,
                                 'in historic period.')) %>%
      ungroup() %>%
      filter(first(.data$scenario) == .data$scenario) %>%
      mutate(pf = sub('^ue_', '', .data$pf), flow = flow) %>%
      select(-'scenario', subsector = pf)
  }

  cement_production <- .get_production('Industry_Value_Added', 'ue_cement',
                                       'NONMET')

  steel_production <- .get_production('Steel_Projections',
                                      c('ue_steel_primary',
  ...
  data_to_fix2 <- inner_join(
    data_industry %>%
      filter(flow %in% c('NONMET', 'IRONSTL')) %>%
      group_by(!!!syms(c('iso3c', 'year', 'flow'))) %>%
    ...
    distinct(!!!syms(c('iso3c', 'year', 'flow')))

  data_industry %>%
    # exclude suspicious data
    anti_join(data_to_fix2, c('iso3c', 'year', 'flow')) %>%
    # add production data
    inner_join(
      bind_rows(cement_production, steel_production) %>%
        group_by(!!!syms(c('iso3c', 'year', 'flow'))) %>%
  ...
  # get years/flows where data needs fixing
  data_to_fix2 %>%
    distinct(!!!syms(c('year', 'flow'))) %>%
    # add industry data for those years/flows
    left_join(
      data_industry %>%
        rename(energy = 'value'),
    ...
    # exclude data that is suspicious
    anti_join(data_to_fix2, c('iso3c', 'year', 'flow')) %>%
    # add production data
    left_join(
      bind_rows(cement_production, steel_production) %>%
    ...
    # duplicate all data for global region
    pivot_longer(c('energy', 'production')) %>%
    duplicate_(list(region = 'World')) %>%
    pivot_wider() %>%
    # calculate regional/global average specific FE demand
  ...
  foo %>%
    right_join(
      data_to_fix2 %>%
        ...
        filter('ALB' == iso3c, 1995 == year),

      c('region', 'year', 'flow')
    ) %>%
    left_join(
      bind_rows(cement_production, steel_production) %>%
        select(-'subsector', production = 'value'),
